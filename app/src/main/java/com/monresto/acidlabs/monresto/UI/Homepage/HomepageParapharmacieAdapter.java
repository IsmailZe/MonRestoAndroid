...
public class HomepageParapharmacieAdapter extends RecyclerView.Adapter<HomepageParapharmacieAdapter.ViewHolder> {
    private Context context;
    private ArrayList<String> images;
    private final static int FADE_DURATION = 1000; //FADE_DURATION in milliseconds

    public HomepageParapharmacieAdapter(Context context) {
    ...
    @NonNull
    @Override
    public HomepageParapharmacieAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v;
        v = LayoutInflater.from(context).inflate(R.layout.item_homepage_extra, viewGroup, false);

        return new HomepageParapharmacieAdapter.ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull HomepageParapharmacieAdapter.ViewHolder viewHolder, int i) {
        String image;
        ...
        if (images != null && !images.isEmpty()) {
            image = images.get(i);
            if (i == 0) {
                viewHolder.item_title.setText("Parapharmacie");
                Picasso.get().load(image).into(viewHolder.item_bg);
                viewHolder.itemContainer.setOnClickListener(e -> {
                    if (!ShoppingCart.getInstance().isEmpty() && !(ShoppingCart.getInstance().getRestoID() == 536 || ShoppingCart.getInstance().getRestoID() == 0)) {
                        ...
                        builder.setPositiveButton("OK", (dialog, which) -> {
                            ...
                            intent.putExtra("EXTRA_SESSION_ID", "536");
